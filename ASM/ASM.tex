...
Es gibt eine Abweichung von wenigen Prozent zwischen der linearisierten und quadratischen Extrapolation. 
Durch die quadratische Extrapolation bekommen Messfehler einen stärkeren Einfluss.
Eine quadratische Extrapolation eignet sich trotzdem um eine anschauliche Gerade zu bilden.
...
folgende Beziehnung: \[ M = \textit{konst.} \cdot U^{2} \]


\section{Bestimmung der Ersatzschaltbildparameter}
\subsection{Parameterbestimmung aufgrund der Kurzschlussmessung}
BErechnung des gemessenen Ständerwiderstands $R_{1}$ um die Maschinenerwärmung auf $T=75^\circ$C zu berücksichtigen:
\begin{align}
...
Maschine geringer. Nimmt man also einen Temperaturanstieg auf nur
55°C kommt man dem gemessenen Wert deutlich näher.
...
Zur Berechnung von $R_{Reib}$ wird die Spannung $U_{h, Fe}$ aus {\ref{eq:UhFe}} verwendet
...
Die starke Abweichung von 136 $ \Omega$ zwischen dem über Strom und Spannung berechneten Widerstand $R_{h, Fe}$ und dem über die Leistung berechneten Widerstand lässt sich zum Teil mit dem sehr ungenauen analogen Wattmeter und dem dadurch entstehenden Paralaxenfehler begründen.
Durch die quadrierung der gemessenen Spannung verstärkt sich zusätzlich der Fehlereinfluss.
...
\input{./tables/klossche_gleichung.tex}
\input{./tables/kippmoment_im_stillstand.tex}

Der eben berechnete Wert hat eine Abweichung von wenigen Prozent, dies lässt sich mit den typischen Messungenauigkeiten begründen. Klar zu sehen ist aber das eine Vernachlässigung von $R_{1}$ zu einem deutlichen Fehler führt.
